// @ts-nocheck
/**
 * Live health board logic (h5.xiaotaozi.cc/health/ build 20260914-1735-ai-now).
 * Data: session-gated /api/me/daily nested days (kedoupi sample in DEV).
 */
declare const Chart: new (...args: unknown[]) => { destroy: () => void };

export type LoginGate = () => void;

let showLoginGate: LoginGate = () => {
  window.location.href = "/";
};

export function startHealthBoard(onUnauthorized?: LoginGate) {
  if (onUnauthorized) showLoginGate = onUnauthorized;
  const chip = document.getElementById("verChip");
  if (chip) chip.textContent = "版 1735";
  load();
}

  const API = "/api/me/daily?days=90";
  const BUILD = "20260914-1735-ai-now+oauth";
  const SLEEP_LABELS = {
    deep_sleep: "深睡",
    efficiency: "效率",
    latency: "入睡",
